...
int main(int argc, char **argv) {
    ...
    if (argc > 2) {
      ...
      // 5 is sufficient for determining sunrise and sundown
      const char* hash = geohash_encode(lat, lng, 5);
      printf ("<%f,%f> -> %s", lat, lng, hash);
    ...
    const char* hash = geohash_encode(42.60498046875, -5.60302734375, 5);
