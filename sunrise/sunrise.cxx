...
//  struct tm *local_hour, *gmt_hour;
 ...
 /*   
    // local time adjust
    // note some zones are 30 min offsets!!!!!
    time( &hour_machine );      // Get time as long integer.
    gmt_hour =  gmtime( &hour_machine );
    h1 = gmt_hour->tm_hour;
    m1 = gmt_hour->tm_min;
    local_hour = localtime( &hour_machine );    // local time.
    h2 = local_hour->tm_hour;
    m2 = local_hour->tm_min;

    float offset = h2 - h1 + (m2 - m1)/60.0;

 */

    // resultat
    result = 12 + fabs(dayDurationj / 2) - diffUTC_TSV + offset;
...
int main(int argc, char* argv[])
{
    ...
    double lon = -118.243683 ;
     const char *hash = geohash_encode (lat, lon, 4);
    ...
    double _CalculateSunsetUniversalTime =  CalculateSunsetUniversalTime
	                        (day, month, year, rlong, rlat);
     cout << "Sunrise UTC: " << _CalculateSunsetUniversalTime << endl;
	     print_time (_CalculateSunsetUniversalTime);
